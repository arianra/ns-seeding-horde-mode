...
#
# Layout
#   <repo> ~/projects/ns-seeding-horde-mode
...
#   D:\games\horde\               runtime state only, nothing authored here
...
# Export target for LaunchPad, on the Windows side. Why not open the repo directly:
# wslpath gives \\wsl.localhost\Ubuntu\... and this WSL instance cannot even list that
# UNC root, so a Windows GUI app browsing it as a project folder is unverified at best -
# and "output cannot be empty" is exactly the kind of failure a path the tool cannot
# resolve would produce. So publish.sh exports a self-contained project to a plain
# D:\ path. One direction only: repo -> export. The export is disposable and generated.
PUBLISH_ROOT_WSL="$HORDE_ROOT_WSL/publish"
PUBLISH_PROJECT_WSL="$PUBLISH_ROOT_WSL/$MOD_NAME"
PUBLISH_PROJECT_WIN="$HORDE_ROOT_WIN\\publish\\$MOD_NAME"
