...
| **G1d** | Boot DEV with `-modstorage <dir>` and confirm the log reports it as the mod-storage directory | boot line reads `Passed '<dir>' as mod-storage directory`; DEV no longer shares `%APPDATA%` mod storage with LIVE |
